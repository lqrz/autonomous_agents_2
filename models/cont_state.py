...
class ContState(State):
    ...
    def prey_is_caught(self):
        ...
        """
        for distance in self.relative_distances:
            if np.sqrt(np.sum(np.power(np.asarray(distance), 2))) <= 1.0:
                return True
        return False

    @classmethod
    def all_states(cls, field, sample_steps=0.01):
        """
        Returns a sampled list of all possible states
        ...
        all_distances = itertools.product(range(-1 * x_lim, x_lim + 1, sample_steps), range(-1 * y_lim, y_lim + 1, sample_steps))
